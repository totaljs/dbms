...
const InMemory = require('total4/inmemory');
...
function select(client, cmd) {
	...
	var conn = INSTANCES[opt.table] || (INSTANCES[opt.table] = InMemory.load(PATH.databases(opt.table + '.inmemory')));
...
function query(client, cmd) {
	...
	var conn = INSTANCES[opt.table] || (INSTANCES[opt.table || opt.db] = InMemory.load(PATH.databases(opt.table || opt.db + '.inmemory')));
